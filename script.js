...
if(findsGrid){findsGrid.innerHTML=rotatingHomepageFinds().map(find=>{const story=allStories.find(item=>item.id===find.storyId);const url=taggedAmazonUrl(find.url);return `<article class="find-card"><a class="find-image" href="${url}" target="_blank" rel="sponsored nofollow noopener"><img src="${find.image}" alt="${find.alt||find.title}" width="600" height="600" loading="lazy"></a><div class="find-copy"><span>RIDICULOUSLY RELEVANT TO</span><a class="find-story-link" href="${storyPath(story)}" data-story="${find.storyId}">${story?.title||'Today’s story'} →</a><h3>${find.title}</h3><p>${find.quip}</p><a class="shop-link" href="${url}" target="_blank" rel="sponsored nofollow noopener">SEE THE EXACT ITEM ON AMAZON ↗</a></div></article>`}).join('')}
const navToggle=document.querySelector('.nav-toggle');const primaryNav=document.querySelector('#primary-nav');if(navToggle&&primaryNav){navToggle.addEventListener('click',()=>{const open=navToggle.getAttribute('aria-expanded')==='true';navToggle.setAttribute('aria-expanded',String(!open));primaryNav.classList.toggle('open',!open)});primaryNav.addEventListener('click',()=>{navToggle.setAttribute('aria-expanded','false');primaryNav.classList.remove('open')})}
